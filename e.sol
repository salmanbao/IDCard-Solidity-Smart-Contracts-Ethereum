...
contract Email {
    ...
    mapping(bytes24 => uint) public wallet;
    ...
    mapping(bytes24 => user) public users;
    ...
    //inbox structure
    mapping(bytes24 => mail[]) public mailbox;
    mapping(bytes24 => uint) public inbox_count;
    
    //sent mail structure
    mapping(bytes24 => sent[]) public sent_list;
    mapping(bytes24 => uint) public sent_count;
    
    //set public key
    function set_public_key(bytes key,uint c_method) public {
        bytes24 name = idc.IDs(msg.sender);
        var (id,addr,profile,about,prestige,level,active,used,index) = idc.info(name);
        require(active == true);
        
        //if Email activated
        if(users[name].used!=true){
            users[name].name = name;
            users[name].used = true;
            users[name].pub_key = key;
            users[name].method = c_method;
        } else {
            users[name].pub_key =key;
            users[name].method = c_method;
        }
    ...
    //set fee
    function set_contact_fee(uint cfee) public {
        bytes24 name = idc.IDs(msg.sender);
        require(users[name].used == true);
        users[name].fee = cfee;
    }
    
    //send Email to people
    function mailto(bytes24 _receiver, string content) public payable {
        require(users[_receiver].used == true);
        bytes24 name = idc.IDs(msg.sender);
        require(users[name].used == true && msg.value >= users[_receiver].fee);
        
        inbox_count[_receiver] += 1;
        mailbox[_receiver].push(mail({
            sender : name,
            message : content,
            reply : "",
            value : msg.value,
        ...
        sent_count[name] += 1;
        sent_list[name].push(sent({
            receiver : _receiver,
            index : inbox_count[_receiver]-1
        }));
    }
    
    //reply an email
    function reply(uint _index,string _reply) public {
        bytes24 name = idc.IDs(msg.sender);
        require(users[name].used == true);
        require(mailbox[name][_index].replied==false);
        mailbox[name][_index].reply = _reply;
        mailbox[name][_index].active = true;
        mailbox[name][_index].replied = true;
        wallet[name] += mailbox[name][_index].value;
    }
    
    function trash(uint _index) public {
        bytes24 name = idc.IDs(msg.sender);
        require(users[name].used == true);
        require(mailbox[name][_index].active==true);
        mailbox[name][_index].active = false;
    }
    
    function untrash(uint _index) public {
        bytes24 name = idc.IDs(msg.sender);
        require(users[name].used == true);
        require(mailbox[name][_index].active==false);
        mailbox[name][_index].active = true;
    }
